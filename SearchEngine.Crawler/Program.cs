...
namespace SearchEngine.Crawler
{
	class Program
	{
		...
		{
			string textFromFile;
			using (FileStream fstream = File.OpenRead(@"C:\Users\Microsoft\source\repos\SearchEngine\urls.txt"))
			{
		...
		private static void SaveHtml(List<IDocument> documents)
		{
			...
			{
				var currentFilePath = Path.Combine(@"C:\Users\Microsoft\source\repos\SearchEngine\выкачка\", $"{index}.txt");
				var awd = item.All
